...
bool sorting_quer(dquery x,dquery y)
{
...
int main() {
	...
	sort(quer,quer+q,sorting_quer);


	int start=quer[0].l;
	int end=quer[0].l;
	cnt[inp[end-1]]++;        //EVERY TIME -1 coz 1 index format
	for(int i=0;i<q;i++)
		{
			...
			while(start<quer[i].l)
			{
				cnt[inp[start-1]]--;
				if(cnt[inp[start-1]]==0)
					res--;
