...
int main() {
	...
	int n;cin>>n;
	vector<int> a,b;
	int max_first=-1,pos=0;
	for(int i=0;i<n;i++)
	{
		int x;cin>>x;
		a.push_back(x);
		if(x>max_first) {max_first=x;pos=i;}
	}
	for(int i=0;i<n;i++)
	{
		int y;cin>>y;
	...
	while(cnt<n-1)
	{
		/*if(cnt==0 && point>a[cnt+1] && n>=3)
		{
			...
			else if(a[cnt+1]<a[cnt+2] && b[cnt+1]>=a[cnt+2])
			{
				point=a[cnt+2];
				cnt++;
			}
		...
		if(a[cnt]<a[cnt+1] && b[cnt]<a[cnt+1])
		{
			sum+=abs(a[cnt+1]-b[cnt]);
			cnt++;
		}
		else if(b[cnt]>b[cnt+1] && a[cnt]>a[cnt+1])
		{

			sum+=abs(a[cnt]-b[cnt+1]);
			cnt++;
		}
		else cnt++;
